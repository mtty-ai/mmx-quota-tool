window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "}",
      ".dsh-mmx-quota-dock__panel-list { padding:6px 0; }",
      ".dsh-mmx-quota-dock__panel .row {",
      "  display:grid;",
      "  grid-template-columns: 88px 1fr;",
      "  column-gap:14px; align-items:center;",
      "  padding:8px 14px; transition:background-color 0.1s ease;",
      "}",
      ".dsh-mmx-quota-dock__panel .row:hover { background:var(--dsw-alias-bg-layer-2, rgba(0,0,0,0.03)); }",
      ".dsh-mmx-quota-dock__panel .row-name {",
      "  font-weight:600; font-size:12px;",
      "  color:var(--dsw-alias-label-primary, #1f2328);",
      "  white-space:nowrap; overflow:hidden; text-overflow:ellipsis;",
      "}",
      ".dsh-mmx-quota-dock__panel .row-stats { display:grid; row-gap:4px; min-width:0; }",
      ".dsh-mmx-quota-dock__panel .stat {",
      "  display:grid;",
      ...
      "}",
      ".dsh-mmx-quota-dock__panel .bar > span { display:block; height:100%; border-radius:99px; transition:width 0.3s ease; }",
      ...
      "  flex:0 0 56px; text-align:right; font-size:10px;",
      "  color:var(--dsw-alias-label-secondary, #9ca3af); font-variant-numeric:tabular-nums;",
      "}",
      ".dsh-mmx-quota-dock__panel .row-warn {",
      "  flex:0 0 auto; padding:10px 14px;",
      "  color:var(--dsw-alias-state-error-primary, #dc2626); font-size:11px;",
      "  background:var(--dsw-alias-bg-layer-1, rgba(220,38,38,0.04));",
      "}",
      ".dsh-mmx-quota-dock__panel-footer {",
      "  padding:8px 14px; border-top:1px solid var(--dsw-alias-border-l1, #e5e7eb);",
      "  display:flex; justify-content:space-between; align-items:center; gap:8px;",
      "  font-size:10px; color:var(--dsw-alias-label-secondary, #6b7280);",
      "}",
    ...
    function QuotaIcon(props) {
      ...
      var panel = open ? React.createElement("div", {
        ...
        React.createElement("div", { className: "dsh-mmx-quota-dock__panel-header" },
          React.createElement("span", null, "MiniMax Token Plan"),
          React.createElement("span", { className: "dsh-mmx-quota-dock__panel-header-meta" }, rows.length + " 个模型"),
        ),
